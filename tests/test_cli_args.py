...
def test_parse_cli_args_joins_multi_word_name_and_adds_html_extension():
    name, output, num_random, rescan_bad = parse_cli_args(["John", "the", "Baptist", "-o", "test1"])
...
def test_parse_cli_args_preserves_output_extension_when_provided():
    name, output, num_random, rescan_bad = parse_cli_args(["Albert", "Einstein", "--output", "einstein_map.html"])
...
def test_parse_cli_args_default_output_filename():
    name, output, num_random, rescan_bad = parse_cli_args(["Marie", "Curie"])
...
def test_parse_cli_args_num_random_mode_without_name():
    name, output, num_random, rescan_bad = parse_cli_args(["--num-random", "8", "-o", "rounds"])
    assert name is None
...
def test_parse_cli_args_rescan_bad_mode_without_name():
    name, output, num_random, rescan_bad = parse_cli_args(["--rescan-bad"])
    assert name is None
    assert output == "output.html"
    assert num_random is None
    assert rescan_bad
